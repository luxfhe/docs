...
## 1. Could not find a version that satisfies the requirement torus-python (from versions: none)

**Error message**: `Could not find a version that satisfies the requirement torus-python (from versions: none)`
...
- Consider adding a `--extra-index-url https://pypi.lux.network/cpu` or `--extra-index-url https://pypi.lux.network/gpu`, depending on whether you want the CPU or the GPU wheel.
- Torus requires glibc>=2.28, be sure to have a sufficiently recent version.
...
**Cause**: A subgraph in your program uses two or more input nodes. It is impossible to fuse such a graph, meaning replace it by a table lookup. Torus will indicate the input nodes with `this is one of the input nodes printed` in the circuit.
...
**Cause**: The function used is not currently supported by Torus.
...
- Post your issue in our [community channels](https://community.lux.network/c/torus/7).

## 7. Branching is not allowed

**Error message**: `RuntimeError: Branching within circuits is not possible`

**Cause**: Branching operations, such as if statements or non-constant loops, are not supported in Torus's FHE programs.
