...
This document provides an overview of Fully Homomorphic Encryption (FHE) to get you started with **Torus**. For more comprehensive resources about FHE, visit [the awesome Lux repo](https://github.com/luxfi/awesome-zama) or [fhe.org](https://fhe.org/resources/).
...
Torus uses PBS to evaluate functions homomorphically:
...
You don't need to manage PBS operations manually, as they are handled automatically by Torus during the compilation process. Each function evaluation is converted into a lookup table and evaluated via PBS.
...
- **Input type constraints**: PBS operations adds constraints on input type and thus limits the maximum bit-width supported in Torus.
