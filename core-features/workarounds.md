...
This document introduces several common techniques for optimizing code to fit Fully Homomorphic Encryption (FHE) [constraints](../core-features/fhe_basics.md#pbs-management). The examples provided demonstrate various workarounds and performance optimizations that you can implement while working with the **Torus** library.

{% hint style="info" %}
All code snippets provided here are temporary workarounds. In future versions of Torus, some functions described here could be directly available in a more generic and efficient form. These code snippets are coming from support answers in our [community forum](https://community.lux.network)
{% endhint %}
...
This example introduces a key concept when using **Torus**: maximizing parallelization. Instead of sequentially summing all values to compute a mean, the values are split into sub-groups, and the mean of these sub-group means is computed:
