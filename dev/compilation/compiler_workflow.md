...
This document explains the different passes happening in the compilation process, from the Torus Python frontend to the Torus MLIR compiler.

There are two main entry points to the Torus Compiler. The first is to use the Torus Python frontend. The second is to use the Compiler directly, which takes [MLIR](https://mlir.llvm.org/) as input. Torus Python is more high level and uses the Compiler under the hood.
...
With the current version of **Torus**, floating-point inputs and floating-point outputs are not supported. However, if the floating-point operations are intermediate operations, they can sometimes be fused into a single table lookup from integer to integer, thanks to some specific transforms.
