...
This document explains how to use GPU accelerations with Torus.

Torus supports acceleration using one or more GPUs.

{% hint style="info" %}
This version is not available on [pypi.org](https://pypi.org/project/torus-python), which only hosts wheels with CPU support. 
{% endhint %}

To use GPU acceleration, install the GPU/CUDA wheel from our [Lux public PyPI repository](https://pypi.lux.network) using the following command:

`pip install torus-python  --extra-index-url https://pypi.lux.network/gpu`.
