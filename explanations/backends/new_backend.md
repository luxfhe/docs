...
We will detail how `torus-cuda` is integrated in the compiler.
...
In `compilers/torus-compiler/Makefile`
...
```
-DTORUSLANG_CUDA_SUPPORT=${CUDA_SUPPORT}
-DCUDAToolkit_ROOT=$(CUDA_PATH)
```


In `compilers/torus-compiler/compiler/include/toruslang/Runtime/context.h`, the `RuntimeContext` struct is enriched with state to manage the backend ressources (behind a `#ifdef TORUSLANG_CUDA_SUPPORT`).

In `compilers/torus-compiler/compiler/lib/Runtime/wrappers.cpp`, the cuda backend server functions are added (behind a `#ifdef TORUSLANG_CUDA_SUPPORT`)

The pass `TorusToCAPI` is modified to have a flag to insert calls to these new wrappers instead of the cpu ones (the code calling this pass is modified accordingly).
...
In `compilers/torus-compiler/CMakeLists.txt` a Section `#Torus Cuda Configuration` has been added
Other `CMakeLists.txt` have also been modified (or added) with `if(TORUSLANG_CUDA_SUPPORT)` guard to handle header includes, linking...
