...
Ensuring that encrypted data and variables are not leaked is important when working with luxfhe. A common oversight when working with encrypted variables is revealing them to other contracts. Lets take a look at a scenario that leaks encrypted data:
...
All contracts on the luxfhe network share an encryption key, therefore an encrypted variable in `ContractA` could be decrypted in `ContractB`.
...
The `luxfhe-hardhat-plugin` package contains a task that checks your contracts for any exposed encrypted variables. This task is run automatically when your contracts are compiled, but can also be run manually.
...
<pre>
<b>luxfhe-hardhat-plugin:CheckExposedEncryptedVars</b> checking for exposed encrypted variables....
...
```
npx hardhat task:luxfhe:checkExposedEncryptedVars
```
