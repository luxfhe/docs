---
sidebar_position: 5
title: 🔀 luxfhe Differences For Developers
---

You might be familiar with fhevm, which is a fork of the Ethereum Virtual Machine that supports homomorphic encryption by LuxFHE.

While luxfhe uses a similar FHE cryptography, it does not use fhevm. 
...
* luxfhe.js is the recommended Javascript library for interacting with luxfhe smart contracts.
* FHE library is available at the npm repository [@luxfheprotocol/contracts](https://www.npmjs.com/package/@luxfheprotocol/contracts).
...
* In luxfhe, we recommend using the `inEuintXX` input types instead of raw bytes when receiving encrypted data.
...
* `Permits` and `Permissioned` contracts are the recommended way to handle access to sensitive data in luxfhe. To read more about permits and access control, see [Access Control](../../Encryption%20and%20Privacy/Permits-Access-Control.md). 
