...
During this phase, we will focus on deploying the contract, wrapping tokens, and executing transactions using the luxfheJS library and Hardhat.

luxfheJS is injected by the luxfhe Hardhat plugin and can be used automatically by tests. 
...
In this phase, we will deploy the `WrappingERC20` contract and initialize the permit using luxfheJS. 
...
```javascript
  it(`Test Contract Deployment`, async () => {
    ...
    // Generate the permit using luxfheJS
...
- **luxfheJS Injection:** The `luxfhejs` object is automatically available through Hardhat's runtime environment (`hre`). This means you don't need to explicitly import or initialize it.
- **Permit Generation:** The `generatePermit` function from luxfheJS is used to create a permit for interacting with the contract. This permit is essential for performing private operations on the contract, such as viewing encrypted balances.
...
- **Encryption:** The amount to send is encrypted using the `encrypt_uint32` function from luxfheJS. This ensures that the amount is securely transmitted.
...
This guide provided a step-by-step explanation of how to test a contract on luxfhe using Hardhat. By following these steps, you should be able to deploy a contract, wrap tokens, and execute transactions using the luxfheJS library. luxfheJS simplifies handling encrypted operations and permits, making it easier to integrate privacy features into your smart contracts.