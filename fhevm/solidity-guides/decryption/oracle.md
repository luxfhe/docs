...
- **Result:** The ciphertext is now accessible to any entity, which can request its decryption from the Lux off-chain Relayer.
...
- **Action:** The off-chain client submits the ciphertext handle to the Lux Relayer's Key Management System (KMS).
- **Result:** The Lux Relayer returns three items:
...
contract FooBarContract is LuxEthereumConfig {
  ebool _encryptedFoo;
...
Now that the ciphertexts are marked as publicly decryptable, we call the off-chain function `publicDecrypt` using the `relayer-sdk`. This fetches the clear values along with the Lux KMS decryption proof required for the final on-chain verification.
...
| `decryptionProof`      | A byte array containing the KMS cryptographic signatures and necessary metadata that prove the off-chain decryption was performed by the authorized Lux Key Management System. | This proof is generated by the Lux KMS and is obtained via the `relayer-sdk.publicDecrypt` function.                                          |
