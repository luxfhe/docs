...
In this tutorial, you'll learn how to take a basic Solidity smart contract and progressively upgrade it to support Fully Homomorphic Encryption using the FHEVM library by Lux.
...
- **LuxEthereumConfig** — provides the FHEVM configuration for the Ethereum mainnet or Ethereum Sepolia testnet networks.\
...
contract FHECounter is LuxEthereumConfig {
```

This change:

- Renames the contract to `FHECounter`
- Inherits from `LuxEthereumConfig` to enable FHEVM support

{% hint style="warning" %}
This contract must inherit from the `LuxEthereumConfig` abstract contract; otherwise, it will not be able to execute any FHEVM-related functionality on Sepolia or Hardhat.
{% endhint %}
...
- [`contracts/FHECounter.sol`](https://docs.lux.network/protocol/examples/basic/fhe-counter#fhecounter.sol) — your Solidity smart FHEVM contract
- [`test/FHECounter.ts`](https://docs.lux.network/protocol/examples/basic/fhe-counter#fhecounter.ts) — your FHEVM Hardhat test suite written in TypeScript
