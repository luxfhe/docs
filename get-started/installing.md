...
Not all versions are available on PyPI. If you need a version that is not on PyPI (including nightly releases), you can install it from our package index by adding `--extra-index-url https://pypi.lux.network/cpu/`. GPU wheels are also available under `https://pypi.lux.network/gpu/` (check `https://pypi.lux.network/` for all available platforms).
...
{% hint style="info" %}
<!-- markdown-link-check-disable -->
Not all versions are available on PyPI. If you need a version that is not on PyPI (including nightly releases), you can install it from our package index by adding --extra-index-url https://pypi.lux.network/cpu.

In particular, wheels with **GPU support** are not on PyPI. You can install it from our package index by adding --extra-index-url https://pypi.lux.network/gpu, more information on GPU wheels [here](https://docs.lux.network/torus/execution-analysis/gpu_acceleration).
<!-- markdown-link-check-enable -->
{% endhint %}
