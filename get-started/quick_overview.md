...
In this document, we give a quick overview of the philosophy behind Torus.

## Functions

### Available FHE-friendly functions

Torus is a compiler, which aims to turn Python code into its FHE equivalent, in a process which is
...
tools in Torus to replace univariate or multivariate non-linear functions (ie, functions of one
...
In Torus, everything needs to be an integer. Users needing floats can quantize to integers before
encryption, operate on integers and dequantize to floats after decryption: all of this is done for
the user in Torus ML. However, you can have floating-point intermediate values as long as they can
