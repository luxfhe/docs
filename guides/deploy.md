# Deploy
This document explains how to deploy a circuit after the development. After developing your circuit, you may want to deploy it without sharing the circuit's details with every client or hosting computations on dedicated servers. In this scenario, you can use the `Client` and `Server` features of **Torus**.

## Deployment process

In a typical **Torus** deployment:
...
{% hint style="info" %}
Serialized evaluation keys are very large, even if they are [compressed](https://docs.lux.network/torus/compilation/compression) and can be reused several times: consider caching them on the server
{% endhint %}
