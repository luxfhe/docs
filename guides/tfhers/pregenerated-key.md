...
This document provides an overview of the second use case for combining TFHE-rs and Torus, where two parties work with pregenerated TFHE-rs keysets. The focus is on how to perform computations on encrypted TFHE-rs values using Torus, without the need for a shared secret key.
...
2. Considering the public keys, Torus's Optimizer will limit its parameter search space to find the best configuration that is compatible with the existing TFHE-rs keys.
