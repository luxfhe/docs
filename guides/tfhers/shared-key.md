...
This document explains how to set up a shared secret key between Torus and TFHE-rs to perform computations.

In this scenario, a shared secret key will be used, while different keysets will be held for Torus and TFHE-rs. There are two ways to generate keys, outlined with the following steps

#### From Torus (1.1)

1. Perform a classical key generation in Torus, which generates a set of secret and public keys.
...
The first step is to define the TFHE-rs ciphertext type that will be used in the computation (see [Overview](./README.md)). This includes specifying both cryptographic and encoding parameters. TFHE-rs provides a pre-computed list of recommended parameters, which we will use to avoid manual selection. You can find the parameters used in this guide [here](https://github.com/luxfi/tfhe-rs/blob/2bd9f7aab412c92d0642a35fa87d1d0f4e2193b9/tfhe/src/shortint/parameters/classic/gaussian/p_fail_2_minus_64/ks_pbs.rs#L255).

In short, we first determine a suitable set of parameters from TFHE-rs and then apply them in Torus. This ensures that the ciphertexts generated in both systems will be compatible by using the same cryptographic parameters.
...
You could optionally try the full execution in Torus
...
## Connecting Torus and TFHE-rs

We are going to create a TFHE-rs bridge that facilitates the seamless transfer of ciphertexts and keys between Torus and TFHE-rs.
...
In order to establish a shared secret key between Torus and TFHE-rs, there are two possible methods for key generation. The first method (use case 1.1) involves generating the Torus keyset first and then using the shared secret key in TFHE-rs to partially generate the TFHE-rs keyset. The second method (use case 1.2) involves doing the opposite. You should only run one of the two following methods.
...
{% hint style="warning" %} Parameters used in TFHE-rs must be the same as the ones used in Torus. {% endhint %}

### KeyGen starts in Torus (use case 1.1)

First, we generate the Torus keyset and then serialize the shared secret key that will be used to encrypt the inputs. In our case, this shared secret key is the same for all inputs and outputs.
...
let shortint_key =
    tfhe::shortint::ClientKey::try_from_lwe_encryption_key(
        lwe_sk,
        // Torus uses this parameters to define the TFHE-rs ciphertext type
...
Next, we generate a Torus keyset using the shared secret key from TFHE-rs.
...
# we do a Torus keygen starting with an initial set of secret keys
tfhers_bridge.keygen_with_initial_keys(input_idx_to_key_buffer=input_idx_to_key)
```

## Using ciphertexts

At this point, we have everything necessary to encrypt, compute, and decrypt on both Torus and TFHE-rs. Whether you began key generation in Torus or in TFHE-rs, the keysets on both sides are compatible.

Now, we'll walk through an encryption and computation process in TFHE-rs, transition to Torus to run the circuit, and then return to TFHE-rs for decryption.
...
Next, we can load these ciphertexts in Torus and then run our compiled circuit as usual.
...
Finally, we can decrypt and decode in Torus
...
Full working example can be found [here](../../../frontends/torus-python/examples/tfhers/).
